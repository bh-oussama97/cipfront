...
export class IdeasListComponent {
  ...
  contreMaitreIdeasDataSource: MatTableDataSource<any> = new MatTableDataSource();
 ...
 selectedContreMaitre :string = 'All';
  constructor(private router : Router,private translate:TranslateService,
   private dataService:DataService,private fileService:FileService,
  private ideaService:IdeaService
   ){}
  ngOnInit(): void {
   this.states.push('All',IdeaState.EXECUTED,IdeaState.PRESELECTED,IdeaState.SELECTED,IdeaState.VALIDATED,IdeaState.WAITING,IdeaState.REFUSED);
   const userJson:UserDto = JSON.parse(localStorage.getItem('userJson')) as UserDto;
   this.ideaService.getIdeasListByResponsible(userJson.matricule).subscribe((response:UserTasksDto)=>{
     ...
     this.chefSegmentNames = Array.from(new Set(this.ideasData.map(idea => { return idea.chefSegment}).filter(ele => ele !== undefined)));
     this.chefSegmentNames.unshift('All');
     this.expertNames = Array.from(new Set(this.ideasData.map(idea => { return idea.expert}).filter(ele => ele !== undefined)));
     this.expertNames.unshift('All');
     this.employeeNames = Array.from(new Set(this.ideasData.map(idea=> {return idea.fullName }).filter(ele=> ele !== undefined)))
     this.employeeNames.unshift('All');      
     this.contreMaitreNames = Array.from(new Set(this.ideasData.map(idea=> {return idea.contreMaitre }).filter(ele=> ele !== undefined)))
   ...
   //   })
   // });
   this.transalatedStates = this.states.map(state=> state.replace(state,"ideasContent."+state));   
 ...
 filterbyExpert()
 {
   if (this.selectedExpert === 'All') {
     ...
     this.filteredDataSource = this.ideasData.filter(item => item.expert === this.selectedExpert);
   }
 }  
 filterByState(){
   if (this.selectedState === 'All') {
     ...
     this.filteredDataSource = this.ideasData.filter(item => item.status === this.selectedState);
   }
 }
 filterbyEmployee()
 {
   if (this.selectedEmployee === 'All') {
     ...
     this.filteredDataSource = this.ideasData.filter(item => item.fullName === this.selectedEmployee);
   }
 }  
 filterByChefSegment()
 {
   if (this.selectedChefSegment === 'All') {
     ...
     this.filteredDataSource = this.ideasData.filter(item => item.chefSegment === this.selectedChefSegment);
   }
 }  
 filterByContreMaitre()
 {
   if (this.selectedContreMaitre === 'All') {
     ...
     this.filteredDataSource = this.ideasData.filter(item => item.contreMaitre === this.selectedContreMaitre);
   }
 }
 
 exportIdeas(){
   let ideasColumns:WorksheetColumn[]=[
     {header:this.translate.instant('ideasContent.ideasListTable.ideaNumber'),key : 'ideaNumber',width:10 },
     {header:this.translate.instant('ideasContent.ideasListTable.initiatorName'),key : 'fullName',width:20},
     {header : this.translate.instant('ideasContent.ideasListTable.description'),key: 'description',width:40},
     {header:this.translate.instant('ideasContent.ideasListTable.state'),key : 'status',width:20},
     {header:this.translate.instant('ideasContent.ideasListTable.date'),key : 'createdAt',width:20},
     {header:this.translate.instant('ideasContent.ideasListTable.contreMaitre'),key : 'contreMaitre',width:20},
     {header:this.translate.instant('ideasContent.ideasListTable.chefSegment'),key : 'chefSegment',width:20},
     {header:this.translate.instant('ideasContent.ideasListTable.expert'),key : 'expert',width:20},
   ];
   this.fileService.exportTableXLSX('ideasSheet',ideasColumns,'idees.xlsx',this.ideasData);
 }
 ...
 searchIdeaByName(event:Event)
 {
   ...
   this.filteredDataSource = this.ideasData.filter(item=> item.description.includes(filterValue))
 }
 ...
 associatedTableAction(e:TableButtonAction):void{
   if (e.name === 'edit' && this.currentRole === 'CONTRE_MAITRE')
   {
     ...
     this.router.navigateByUrl('ideas/preselection/'+e.value.ideaId);
   }
   if(e.name === 'edit' && this.currentRole === 'CHEF_SEGMENT')
   {
     //this.dataService.transfertObject(e.value);
     this.router.navigateByUrl('ideas/selection/'+e.value.ideaId);
   }
   if(e.name === 'edit' && this.currentRole === 'EXPERT')
     {
       //this.dataService.transfertObject(e.value);
       this.router.navigateByUrl('ideas/execution/'+e.value.ideaId);
     }
   if (e.name === 'details')
   {            
    //  this.dataService.transfertObject(e.value);
